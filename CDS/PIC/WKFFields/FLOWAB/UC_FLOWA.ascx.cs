...
/// 統一 - 請訂驗流程
/// </summary>
public partial class WKF_OptionalFields_CDS_PIC_WKFFields_UC_FLOWA : WKF_FormManagement_VersionFieldUserControl_VersionFieldUC
{
    /// <summary>
    /// 資料庫連通字串
    /// </summary>
    ...
    private DataTable CreateAllRelationForms()
    {
        ...
        dtReturn.Columns.Add(new DataColumn("FLOWB_NO", typeof(String))); // 表單單號
        dtReturn.Columns.Add(new DataColumn("FLOWB_TASK_ID", typeof(String))); // B單的TASK_ID
        dtReturn.Columns.Add(new DataColumn("FLOWB_STATUS", typeof(String))); // 簽核狀態
    ...
    private DataTable GetAllRelationForms()
    {
        ...
        using (SqlDataAdapter sda = new SqlDataAdapter(@"
               SELECT FLOWB_NBR AS 'FLOWB_NO', -- 找到所有關聯的資料
                      ISNULL((SELECT TOP 1 TASK_ID 
                                FROM TB_WKF_TASK 
                               WHERE DOC_NBR = ZPIC_AB_TASK.FLOWB_NBR), '') AS 'FLOWB_TASK_ID',
                      ISNULL((SELECT TOP 1 CUSTOM_WORDS 
				                FROM TB_WKF_TASK_NODE 
				               WHERE TASK_ID = (SELECT TOP 1 TASK_ID 
                                                  FROM TB_WKF_TASK 
                                                 WHERE DOC_NBR = ZPIC_AB_TASK.FLOWB_NBR)
				                 AND FINISH_TIME IS NULL), '') AS 'CUSTOM_WORDS', -- 用TASK_ID 取出尚未結案的一筆節點的CUSTOM_WORD
		              CASE WHEN TASK_STATUS_B = 1  -- 簽核狀態
				            AND TASK_RESULT_B IS NULL 
			               THEN '簽核中'
			               WHEN TASK_STATUS_B = 2 
				            AND TASK_RESULT_B = 0 
			               THEN '核准'
			               WHEN TASK_STATUS_B = 2 
				            AND TASK_RESULT_B = 1 
			               ...
			               WHEN TASK_STATUS_B = 2 
				            AND TASK_RESULT_B = 2 
		   	               THEN '作廢'
			               WHEN TASK_STATUS_B = 3 
			               THEN '異常'
			               WHEN TASK_STATUS_B = 4 
			               THEN '退回'
						   WHEN TASK_STATUS_B = 5 
			               THEN '作廢'
			               ELSE '錯誤' 
			                END AS 'FLOWB_STATUS'
                  FROM ZPIC_AB_TASK
	             WHERE FLOWA_NBR = @FLOWA_NBR
              ORDER BY FLOWB_NBR ASC
            ", ConnectionString))
        using (DataSet ds = new DataSet())
        {
            sda.SelectCommand.Parameters.AddWithValue("@FLOWA_NBR", taskObj.FormNumber);
            try
            {
                if (sda.Fill(ds) > 0)
            ...
            {
                DebugLog.Log(DebugLog.LogLevel.Error, string.Format(@"UC_FLOWA.GetAllRelationForms.SELECT.ERROR:{0}", e.Message));
            }
        }
        return dtReturn;
    }
    ...
    protected void gvForms_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
    {
        ...
        if (gr.RowType == DataControlRowType.DataRow)
        {
            ...
            //Label lblFLOWA_NO = gr.FindControl("lblFLOWA_NO") as Label;
            LinkButton lbtnFLOWB_NO = gr.FindControl("lbtnFLOWB_NO") as LinkButton;

            Label lblFLOWB_STATUS = gr.FindControl("lblFLOWB_STATUS") as Label;

            try
            {
                JObject joComment = JObject.Parse(row["CUSTOM_WORDS"].ToString());
                string _status = joComment["zh-TW"].ToString();
                lblFLOWB_STATUS.Text = !string.IsNullOrEmpty(_status) ? _status : row["FLOWB_STATUS"].ToString();
            }
            catch
            {
                lblFLOWB_STATUS.Text = row["FLOWB_STATUS"].ToString();
            }
            ...
            Dialog.Open2(lbtnFLOWB_NO, url, row["FLOWB_NO"].ToString(), 960, 720, Dialog.PostBackType.None, new { TASK_ID = row["FLOWB_TASK_ID"].ToString() }.ToExpando());
        }
    }
}
